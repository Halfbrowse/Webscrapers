    ...
    def parse(self, response):
        for product in response.xpath("//ul[@class='productlisting-ul']/div/li"):
            yield{
                ...
                "dicount_price": product.xpath(".//span[@class='productSpecialPrice fl']/text()").get(),
                "og_price": product.xpath(".//span[@class='normalprice fl']/text()").get()

            }

        next_page = response.xpath("//a[@class='nextPage']/@href").get()
# also add the headers to below after the callback method.
        if next_page:
